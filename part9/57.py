...
# 입력: 질문, 정답, 오답; 질문에 대한 답
# trivia question from quizoid (ios app)
# 프로세스: 정답을 맞춘 횟수를 기록한다, 오답을 선택하면 게임을 종료한다
# 출력: 정답을 맞춘 횟수
...
def read_question():
	with open('trivia.txt', 'rt') as fin:
		...
		question_list2 = [ question_list[i:i+6] for i in range(0, len(question_list), 6) ]
...
def play_game(question_list):
	...
	while True:
		while i < length:
			...
			fmt = "\n{}\n[a] {}\n[b] {}\n[c] {}\n[d] {}"
			print(fmt.format(question_list[question_num][0], question_list[question_num][1], question_list[question_num][2], question_list[question_num][3], question_list[question_num][4]))
