...
def run_server():
	...
	def index():
		...
		if request.method == 'POST':
			...
			if is_url_valid(original_url):
				...
				return render_template('54_2.html', original_url=original_url, shortened_url=full_shortened_url)
			else:
	...
	def stats(shortened_url):
		url_list = read_url()
		url_dict = { url['shortened_url']: [url['original_url'], url['used_times']] for url in url_list }
		original_url = url_dict[shortened_url][0]
		used_times = url_dict[shortened_url][1]
		return render_template('54_4.html', original_url=original_url, shortened_url=shortened_url, used_times=used_times)
...
def increase_used_time(shortened_url):
	url_db = db.reference('url').get()
	url_dict = { url_db[item]['shortened_url']: [item, url_db[item]['used_times']] for item in url_db }
