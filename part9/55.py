...
# 입력: 텍스트
# 프로세스: 텍스트를 데이터 저장소에 저장한다, 저장된 텍스트를 조회할 수 있는 url을 생성한다
# 출력: 텍스트
...
# Private/55_config.ini example:
#
# [firebase]
# key_path=<path to your json key>
...
def run_server():
	...
	def view(url):
		if request.method == 'GET':
			...
			return render_template('55_view.html', title=title, content_list=content_list)
		if request.method == 'POST':
	...
	def edit(url):
		if request.method == 'GET':
			...
			title = text_dict[url][1]
			content_list = text_dict[url][2]
			return render_template('55_edit.html', title=title, content_list=content_list)
		if request.method == 'POST':
...
def read_text():
	...
	if text_db != None:
		text_dict = { text_db[item]['url']: [item, text_db[item]['title'], text_db[item]['content']] for item in text_db }
		return text_dict
	return {}
