...
def remove_todo(done_todo, todo_db):
	todo_dict = { todo_db[item]['todo'][i]: item for item in todo_db for i in range(0, len(todo_db[item]['todo'])) }
	todo_list = todo_db[todo_dict[done_todo]]['todo']
	todo_list.remove(done_todo)
	if len(todo_list) == 0:
		db.reference('/todo/').child(todo_dict[done_todo]).delete()
	else:
		db.reference('/todo/').child(todo_dict[done_todo]).update({
			'todo': todo_list
		})
